    ...
    def Execute(self):
        ...
        if ext not in supportedExts:
            if len(supportedExts) > 1:
                ...
                csv_err = _('Source file must be a %s file or a plugin (%s).'
                                ) % (self.csvFile, plugin_exts)
            else:
                csv_err = _(u'Source file must be a %s file.') % self.csvFile
