...
from ..bolt import AFile, CIstr, FName, GPath, ListInfo, Path, SubProgress, \
    deprint, dict_sort, forward_compat_path_to_fn, GPath_no_norm, \
...
class Installer(ListInfo):
    ...
    def calc_crcs(to_calc, pending_size, rootName, new_sizeCrcDate, progress):
        ...
        for rpFile, (siz, asFile, date) in dict_sort(to_calc):
            ...
            new_sizeCrcDate[rpFile] = (siz, final_crc, date)
...
class InstallerProject(_InstallerPackage):
    ...
    def _file_changed(self, stat_tuple):
        ...
        return self.file_mod_time != max_mtime or self.fsize != proj_size or \
            cached.keys() != size_apath_date.keys() or any(
                size_apath_date[k][0] != ls[0] or size_apath_date[k][2] != ls[
                    2] for k, ls in cached.items())
    ...
    def _refreshSource(self, progress, stat_tuple=None,
        ...
        Installer.calc_crcs(to_calc, pending_size, rootName,
                            size_apath_date, progress) ## fixme check dict_sort still works (in case pending == size_apath_date)
...
class InstallersData(DataStore):
    ...
    def irefresh(self, progress=None, what=u'DIONSC', fullRefresh=False,
        ...
        if 'I' in what:
            ...
            # parameters are None, the Installers dir will be scanned for
            # changes. Note that if any of those are not None "changed" will
            # be always True, triggering the rest of the refreshes in irefresh
            progress = progress or bolt.Progress()
            #--Current archives
            if refresh_info is None:
                if deleted or pending:
                    refresh_info = self._RefreshInfo(deleted, pending,
                                                     projects)
                    pending, projects = refresh_info.pending, \
                        refresh_info.projects
                    dirs_files = (pending & projects, pending - projects)
                else:
                    dirs_files = top_level_items(bass.dirs['installers'])
                progress(0, _('Scanning Packages...'))
                refresh_info = self.scan_installers_dir(*dirs_files,
    ...
    def scan_installers_dir(self, folders, files, fullRefresh, progress, *,
        ...
        for items, is_proj in ((files, False), (folders, True)):
            for item in items:
                ...
                index += 1
                inst = self.get(item)
                if (bug := inst.fn_key != item) or inst is None:
                    if bug:  # some rename bug - corrupted
        ...
        if refresh_info is None:
            deleted = set(self.ipackages(self)) - installers - _refreshed - \
                      _added
    ...
    def _refresh_from_data_dir(self, progress, recalculate_all_crcs):
        ...
        changes = bool(to_calc) or (len(new_sizeCrcDate) != len(
